...
public class LightsaberAssemblerCategory implements IRecipeCategory<LightsaberRecipe> {
    ...
    public void draw(LightsaberRecipe recipe, IRecipeSlotsView recipeSlotsView, PoseStack poseStack, double mouseX, double mouseY) {
        ...
        GuiComponent.drawString(poseStack, GameInstance.getClient().font, I18n.get(MATERIALS_KEY), 0, 78, 0x7E7E7E);
        var lT = LightsaberTag.getOrCreate(recipe.item());
        var titleX = this.window.getWidth() / 2;
        GuiComponent.drawCenteredString(poseStack, GameInstance.getClient().font, StringUtil.makeLightsaberName(recipe.id().getPath()) + "'s Lightsaber", titleX, 5, 0xFFFFFFFF);
        GraphicsUtil.renderLightsaber(poseStack, lT.getTemporary(false, true), titleX - 18, 35, -45f);
    }
}