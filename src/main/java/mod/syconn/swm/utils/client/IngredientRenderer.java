...
public class IngredientRenderer<T extends StackedIngredient> {
    ...
    public void display(GuiGraphics guiGraphics, int count, int x, int y, int mouseX, int mouseY) {
        if (this.displayStacks.size() > this.selectedStack) {
            ...
            if (count > 0) guiGraphics.renderItemDecorations(Minecraft.getInstance().font, new ItemStack(this.displayStacks.get(this.selectedStack).getItem(), count), x, y);
            if (inBounds(x, y, mouseX, mouseY)) guiGraphics.renderTooltip(Minecraft.getInstance().font, new ItemStack(this.displayStacks.get(this.selectedStack).getItem()), mouseX, mouseY);
        }
    }
